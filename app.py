...
if selection:
    ...
    with st.expander("Sections 1-6 Audit Details", expanded=True):
        st.write("1. **Pitching:** Statistical regression analysis signals an advantage for the " + ("favorite" if match_data.iloc[0]['Edge'] > 0 else "underdog") + ".")
        st.write("2. **Bullpen:** Leverage efficiency metrics favor the side with the higher win probability.")
        st.write("3. **Offense:** wRC+ splits against handedness indicate superior rally sustainability.")
        st.write("4. **Game Flow:** Umpire zone and stadium factors analyzed; neutral variance expected.")
